...
<?php
    foreach($usersPaginations as $user) {
        if(!$user["is_admin"]) {
            echo '
                                <div class="add-admin field-container delete-field display-flex flex-row space-between" data-user_id="' . $user["user_id"] . '">
                                    <span>' . $user["username"] . '</span>
                                    <button class="crud-button las la-user-plus" type="button" aria-label="Adicionar"></button>
                                </div>
            ';
        }
    }
